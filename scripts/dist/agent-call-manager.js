...
function detectDependencyCycle(tasks) {
  ...
  const dfs = (node) => {
    if (visiting.has(node)) {
      const idx = path3.indexOf(node);
      return idx >= 0 ? [...path3.slice(idx), node] : [node, node];
    }
...
function validatePlannerOutput(output, header) {
  const issues = [];
  ...
  for (const [index, raw] of tasks.entries()) {
    ...
    if (typeof raw.executionSpec !== "undefined") {
      if (!isPlainObject(raw.executionSpec)) {
        ...
        const executionSpec = raw.executionSpec;
