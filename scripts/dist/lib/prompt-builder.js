...
exports.generateActivationRules = generateActivationRules;
exports.generateRuleActivationPrompt = generateRuleActivationPrompt;
exports.generateWorkspacePrompt = generateWorkspacePrompt;
exports.generateDemoWelcomeBanner = generateDemoWelcomeBanner;
exports.generateUnifiedRoutingPrompt = generateUnifiedRoutingPrompt;
exports.generateDemoQuickActionGuide = generateDemoQuickActionGuide;
exports.generateDemoRuntimeSummary = generateDemoRuntimeSummary;
...
const ACTIVATION_GROUP_DEFINITIONS = [
...
const AGENT_ACTIVATION_CATEGORY_OVERRIDES = {
...
function buildActivationGroups(skills, agents, skillsRootDir, agentsRootDir) {
    ...
    const groups = new Map(ACTIVATION_GROUP_DEFINITIONS.map(definition => [
        definition.key,
        { key: definition.key, title: definition.title, entries: [], mergedTriggers: [] },
    ]));
    for (const agent of [...agents].sort((a, b) => a.id.localeCompare(b.id))) {
        const category = classifyAgentForActivation(agent);
        (_a = groups.get(category)) === null || _a === void 0 ? void 0 : _a.entries.push({
            id: agent.id,
            entityType: 'agent',
            name: agent.name,
    ...
    for (const skill of [...skills].sort((a, b) => a.id.localeCompare(b.id))) {
        ...
        (_b = groups.get(category)) === null || _b === void 0 ? void 0 : _b.entries.push({
            id: skill.id,
            entityType: 'skill',
            name: skill.name,
...
function generateActivationRules(skills, agents, skillsRootDir, agentsRootDir) {
    ...
    for (const group of groups) {
        ...
        const directSkillPaths = group.entries
            .filter((entry) => entry.entityType === 'skill')
            .map(entry => entry.filePath);
        const relatedSkillPaths = agentEntries.flatMap(agent => (agent.relatedSkills || [])
...
function generateDemoWelcomeBanner(options) {
    ...
    if (options.vueVersion)
        pushPart(`Vue ${options.vueVersion}`);
    if (options.lang !== 'unknown')
        pushPart(options.lang === 'typescript' ? 'TypeScript' : options.lang);
    for (const lib of options.uiLibs)
        pushPart(lib);
...
function generateDemoRuntimeSummary(scriptsCount, workflowsCount, taskbooksCount, commandsCount) {
    const items = [];
    if (scriptsCount > 0)
        items.push(`工具脚本 ${scriptsCount} 个`);
    if (workflowsCount > 0)
        items.push(`工作流 ${workflowsCount} 个`);
    if (taskbooksCount > 0)
        items.push(`任务书契约 ${taskbooksCount} 个`);
    if (commandsCount > 0)
        items.push(`命令 ${commandsCount} 个`);
    if (items.length === 0)
        return '';
    return `
## 已安装运行时
