...
var TASKBOOK_TYPES = /* @__PURE__ */ new Set(["new-feature", "refactoring", "debugging", "testing", "code-review"]);
var TASKBOOK_TASK_TYPES = /* @__PURE__ */ new Set(["requirement", "prd", "analysis", "design", "test", "implement", "refactor", "review", "build-fix", "acceptance"]);
var PLANNER_TASK_TYPES = /* @__PURE__ */ new Set(["analysis", "design", "test", "implement", "review"]);
var TASK_STATUSES = /* @__PURE__ */ new Set(["pending", "in_progress", "done", "blocked", "skipped"]);
var TASK_PRIORITIES = /* @__PURE__ */ new Set(["critical", "high", "medium", "low"]);
var TASK_AGENT_HINTS = /* @__PURE__ */ new Set(["coder", "tester", "reviewer", "refactor", "doc-writer", "planner"]);
var BUILTIN_WORKFLOW_IDS = /* @__PURE__ */ new Set(["micro", "sprint", "default"]);
var TASK_SPEC_MODES = /* @__PURE__ */ new Set(["inline-open-spec", "linked-spec-kit"]);
var CHANGE_TYPES = /* @__PURE__ */ new Set(["added", "modified", "removed", "reordered"]);
var AGENT_CALL_STATUSES = /* @__PURE__ */ new Set(["success", "failed", "blocked"]);
...
var KNOWN_WORKFLOW_GATE_TYPES = /* @__PURE__ */ new Set(["checks", "review"]);
...
function listJsonFiles(dirPath, opts) {
  try {
    ...
    return entries.filter((e) => e.isFile()).map((e) => e.name).filter((name) => name.endsWith(".json")).filter((name) => !opts?.exclude?.has(name)).map((name) => path.join(dirPath, name));
  } catch {
    return [];
  }
}
...
function validateTaskScope(scope, file, basePath) {
  const issues = [];
  ...
  for (const scopeKey of ["files", "modules", "tags"]) {
...
function validateExecutionSpec(spec, file, basePath, opts) {
  const issues = [];
  const error = (message) => issues.push({ level: "error", file, message });
  ...
  for (const listKey of ["deliverables", "verification", "constraints"]) {
...
function validateTaskBookPlan(plan, file, taskBookId) {
  const issues = [];
  const error = (message) => issues.push({ level: "error", file, message });
  const warn = (message) => issues.push({ level: "warning", file, message });
  ...
  if (typeof plan.risks !== "undefined") {
    if (!Array.isArray(plan.risks)) {
      ...
      for (const [index, rawRisk] of plan.risks.entries()) {
        ...
        if (!isNonEmptyString(rawRisk.level) || !(/* @__PURE__ */ new Set(["low", "medium", "high"])).has(rawRisk.level)) {
...
function validatePlannerAgentCallOutput(output, file, header) {
  const issues = [];
  const error = (message) => issues.push({ level: "error", file, message });
  const warn = (message) => issues.push({ level: "warning", file, message });
...
function validateTaskBook(data, file) {
  ...
  if ("plan" in data && typeof data.plan !== "undefined") {
    issues.push(...validateTaskBookPlan(data.plan, file, isNonEmptyString(data.id) ? data.id : void 0));
  }
  const taskIdSet = /* @__PURE__ */ new Set();
  const parsedTasks = [];
  if ("tasks" in data) {
...
function parseArgs(argv) {
  ...
  if (!parsed.validateWorkflows && !parsed.validateTaskbooks && !parsed.validateAgentCalls && parsed.workflowPaths.length === 0 && parsed.taskBookIds.length === 0 && parsed.agentCallRequestIds.length === 0) {
...
function batchingScopeIssues(taskBookData, file, opts) {
  ...
  if (missing.length > 0) {
    const examples = missing.slice(0, 5).map((t) => `${t.id}:${t.title || t.type}`).join(", ");
