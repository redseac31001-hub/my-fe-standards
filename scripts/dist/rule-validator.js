...
exports.validateRulesDir = validateRulesDir;
exports.finalizeRuleValidation = finalizeRuleValidation;
...
function validatePriorityValue(value, file) {
    if (!value)
        return null;
    if (/\b(critical|high|medium|low)\b/i.test(value))
        return null;
    return {
        level: 'warning',
        file,
...
function validateRuleFile(filePath, rootDir) {
    ...
    if (fm.frontmatter) {
        ...
        const priorityIssue = validatePriorityValue(((_c = meta.priority) !== null && _c !== void 0 ? _c : '').trim(), rel);
        if (priorityIssue)
    ...
    const fenceMatches = (_d = raw.match(/^```/gm)) !== null && _d !== void 0 ? _d : [];
