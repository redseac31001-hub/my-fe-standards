...
exports.workflowRoutingReportPath = void 0;
exports.resolveWorkflowSelection = resolveWorkflowSelection;
exports.runOrchestratorOnce = runOnce;
...
var workflow_routing_selection_2 = require("./lib/workflow-routing-selection");
Object.defineProperty(exports, "workflowRoutingReportPath", { enumerable: true, get: function () { return workflow_routing_selection_2.workflowRoutingReportPath; } });
...
function buildNextCommand(params) {
    ...
    if (params.tasksOnly)
        parts.push('--tasks-only');
    if (shouldPreserveWorkflowPath(params.workflowPath)) {
        parts.push('--workflow', params.workflowPath.trim());
    }
    if (params.showWorkflowRoute)
        parts.push('--show-workflow-route');
    for (const gateId of (_a = params.approve) !== null && _a !== void 0 ? _a : []) {
        parts.push('--approve', gateId);
    }
    return parts.map(quoteCommandArg).join(' ');
}
function printWorkflowRoute(details, showReasons) {
    console.log(`[Orchestrator] workflow route: ${details.workflowId} (${details.mode}, confidence=${details.confidence}) -> ${details.workflowPath}`);
...
function resolveWorkflowSelection(params) {
    return (0, workflow_routing_selection_1.selectWorkflowForTaskBook)(params);
}
...
function emitOutcome(outcome, json) {
    ...
    const details = 'details' in outcome ? ((_a = outcome.details) !== null && _a !== void 0 ? _a : {}) : {};
    ...
    if (outcome.status === 'completed') {
        ...
        if (workflowRoute)
            printWorkflowRoute(workflowRoute, showWorkflowRoute);
        return;
    }
...
if ((0, cli_entry_1.isDirectCliEntry)('task-orchestrator.js')) {
    main();
}
