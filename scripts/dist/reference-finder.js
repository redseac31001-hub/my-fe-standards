...
function collectSourceFiles(dir, collected, depth = 0) {
    if (depth > 15 || collected.length >= MAX_FILES_SCAN)
        return;
    let entries;
    try {
    ...
    for (const entry of entries) {
        if (collected.length >= MAX_FILES_SCAN)
            break;
...
function searchFileForReferences(filePath, importPatterns, symbolPattern, targetAbsPath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const pattern of importPatterns) {
            ...
            if (match) {
                const kind = /\brequire\s*\(/.test(line) ? 'require'
                    : /\bfrom\s+['"]/.test(line) ? 'from'
                        : /\bimport\s/.test(line) ? 'import'
                            : 'usage';
                results.push({
                    filePath,
                    line: i + 1,
...
function findReferences(target, projectRoot, options) {
    ...
    const maxResults = (_a = options === null || options === void 0 ? void 0 : options.maxResults) !== null && _a !== void 0 ? _a : 100;
    // 收集源文件
    const sourceFiles = [];
    ...
    const symbolPattern = (options === null || options === void 0 ? void 0 : options.symbol) ? buildSymbolPattern(options.symbol) : null;
    // 如果 target 不是文件路径，当作符号搜索
    if (!targetAbsPath && !symbolPattern) {
        const fallbackSymbol = buildSymbolPattern(target);
        const allRefs = [];
        for (const file of sourceFiles) {
            if (allRefs.length >= maxResults)
                break;
...
if ((0, cli_entry_1.isDirectCliEntry)('reference-finder.js')) {
    main();
}
