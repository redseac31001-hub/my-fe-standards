...
function parseRouterConfig(srcPath) {
  ...
  const routerPaths = [
    path6.join(srcPath, "router/index.ts"),
    path6.join(srcPath, "router/index.js"),
    path6.join(srcPath, "router/routes.ts"),
    path6.join(srcPath, "router/routes.js"),
    path6.join(srcPath, "routes/index.ts"),
    path6.join(srcPath, "routes/index.js")
  ];
  for (const routerPath of routerPaths) {
    if (fs5.existsSync(routerPath)) {
      try {
        const content = fs5.readFileSync(routerPath, "utf-8");
...
function scanModules(srcPath, config, maxDepth, routeMap) {
  ...
  for (const pattern of config.modulePatterns) {
    const moduleDirPath = path6.join(srcPath, pattern.pattern);
    if (!fs5.existsSync(moduleDirPath)) continue;
    const stat = fs5.statSync(moduleDirPath);
    if (!stat.isDirectory()) continue;
    if (pattern.recursive) {
      const subDirs = fs5.readdirSync(moduleDirPath);
      for (const subDir of subDirs) {
        if (shouldIgnore(subDir, config.ignorePatterns)) continue;
        const subDirPath = path6.join(moduleDirPath, subDir);
        const subStat = fs5.statSync(subDirPath);
...
function collectModuleStats(dirPath, config, maxDepth, currentDepth) {
  ...
  for (const entry of entries) {
    ...
    const entryPath = path6.join(dirPath, entry);
    let entryStat;
    try {
      entryStat = fs5.statSync(entryPath);
    } catch {
      continue;
    }
    if (entryStat.isFile()) {
...
function findEntries(modulePath, config) {
  const entries = [];
  try {
    const files = fs5.readdirSync(modulePath);
    for (const file of files) {
      const filePath = path6.join(modulePath, file);
...
function findSubModulesDetailed(modulePath, parentModuleName, config, maxDepth) {
  const subModules = [];
  try {
    const entries = fs5.readdirSync(modulePath);
    for (const entry of entries) {
      if (shouldIgnore(entry, config.ignorePatterns)) continue;
      const entryPath = path6.join(modulePath, entry);
  ...
  function dfs(node) {
    ...
    for (const edge of outEdges) {
      if (!visited.has(edge.to)) {
        ...
        const cycleStart = path7.indexOf(edge.to);
        if (cycleStart !== -1) {
          cycles.push([...path7.slice(cycleStart), edge.to]);
        }
      }
    }
    path7.pop();
...
function analyzeModules(options) {
  ...
  for (const dir of possibleSrcDirs) {
    const candidatePath = path6.join(fullPath, dir);
    if (fs5.existsSync(candidatePath) && fs5.statSync(candidatePath).isDirectory()) {
...
function saveReports(targetPath, result) {
  try {
    ...
    const modulesDir = path6.join(reportsPath, "modules");
    if (!fs5.existsSync(modulesDir)) {
      fs5.mkdirSync(modulesDir, { recursive: true });
    }
    const markdownContent = formatMarkdown(result);
    fs5.writeFileSync(path6.join(modulesDir, "latest.md"), markdownContent, "utf-8");
    if (result.mermaidGraph) {
      fs5.writeFileSync(path6.join(modulesDir, "dependency-graph.mmd"), result.mermaidGraph, "utf-8");
    }
