...
exports.parseTaskIntakeCliArgs = parseTaskIntakeCliArgs;
exports.routeTaskIntakeCli = routeTaskIntakeCli;
const cli_entry_1 = require("./lib/cli-entry");
const task_intake_routing_1 = require("./lib/task-intake-routing");
...
function flagAsNumber(flags, key) {
...
输出字段:
  ...
  - documentationTier         minimal | standard | full
  - documentationArtifacts    建议留存的文档/报告清单
...
function buildInput(parsed) {
    var _a, _b;
    const description = ((_a = flagAsString(parsed.flags, 'description')) !== null && _a !== void 0 ? _a : parsed.positionals.join(' ').trim()) || null;
    const title = (_b = flagAsString(parsed.flags, 'title')) !== null && _b !== void 0 ? _b : (description ? description.slice(0, 80) : null);
    return (0, task_intake_routing_1.createDefaultTaskIntakeInput)({
        title,
        description,
        kind: (0, task_intake_routing_1.normalizeTaskIntakeKind)(flagAsString(parsed.flags, 'kind')),
        contractState: (0, task_intake_routing_1.normalizeContractState)(flagAsString(parsed.flags, 'contract')),
        uncertainty: (0, task_intake_routing_1.normalizeUncertainty)(flagAsString(parsed.flags, 'uncertainty')),
...
function printDecision(decision) {
    console.log(`[task-intake-router] response lane: ${decision.recommendedResponseMode} (complexity=${decision.complexityTier})`);
    console.log(`[task-intake-router] documentation: ${decision.documentationTier} -> ${decision.documentationArtifacts.join(', ')}`);
    console.log(`[task-intake-router] recommendation: ${decision.recommendedPath} (confidence=${decision.confidence}, kind=${decision.inferredKind})`);
    console.log(`[task-intake-router] planner contract: workflow=${decision.recommendedWorkflowId}, specMode=${decision.recommendedSpecMode}`);
...
function parseTaskIntakeCliArgs(args) {
    return parseCli(args);
}
function routeTaskIntakeCli(parsed) {
    return (0, task_intake_routing_1.routeTaskIntake)(buildInput(parsed));
}
function main() {
    ...
    const decision = (0, task_intake_routing_1.routeTaskIntake)(input);
...
if ((0, cli_entry_1.isDirectCliEntry)('task-intake-router.js')) {
    main();
}
