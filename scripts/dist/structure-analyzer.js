...
function loadConfig(targetPath, customConfigPath) {
  ...
  const projectConfigPath = path6.join(targetPath, ".structure-analyzer.json");
  ...
  const globalConfigPath = path6.resolve(__dirname, "../../config/loader-config.json");
...
function scanDirectory(dirPath, config, context, maxDepth) {
  ...
  if (stat.isDirectory()) {
    ...
    let entries;
    try {
      entries = fs5.readdirSync(dirPath);
    } catch {
      return null;
    }
    const children = [];
    for (const entry of entries) {
      const childPath = path6.join(dirPath, entry);
...
function checkSA005(node, config) {
  ...
  const pathParts = node.path.split(path6.sep);
...
var SOURCE_FILE_EXTENSIONS = /* @__PURE__ */ new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".vue"]);
...
var COVERAGE_REPORT_FILES = [
  path6.join("coverage", "coverage-summary.json"),
  path6.join("coverage", "lcov.info")
];
var LINT_REPORT_FILES = [
  "eslint-report.json",
  path6.join("reports", "eslint-report.json"),
  path6.join(".codebuddy", "reports", "eslint-report.json")
];
function isSourceCodeFile(filePath) {
  return SOURCE_FILE_EXTENSIONS.has(path6.extname(filePath).toLowerCase());
}
function isTypeScriptFile(filePath) {
  return TYPESCRIPT_EXTENSIONS.has(path6.extname(filePath).toLowerCase());
}
...
function findFirstExistingFile(targetPath, candidates) {
  for (const relativePath of candidates) {
    const fullPath = path6.join(targetPath, relativePath);
...
function hasAnyScriptMatch(scripts, pattern) {
  if (!scripts) {
    return false;
  }
  return Object.values(scripts).some((script) => pattern.test(script));
}
function collectFilesRecursively(dirPath, depth = 0, maxDepth = 4) {
  if (!fs5.existsSync(dirPath) || depth > maxDepth) {
    return [];
  }
  let stat;
  try {
  ...
  const ignored = /* @__PURE__ */ new Set(["node_modules", "dist", ".git", ".codebuddy", "coverage"]);
  const files = [];
  for (const entry of fs5.readdirSync(dirPath)) {
    if (ignored.has(entry)) {
      continue;
    }
    files.push(...collectFilesRecursively(path6.join(dirPath, entry), depth + 1, maxDepth));
  }
  return files;
}
...
function extractRulesCountFromEslintConfig(packageJson, targetPath) {
  ...
  if (eslintConfigPath.endsWith(".json") || eslintConfigPath.endsWith(".eslintrc")) {
    const parsed = safeReadJsonFile(eslintConfigPath);
...
function readLintViolationCount(targetPath) {
  for (const relativePath of LINT_REPORT_FILES) {
    const fullPath = path6.join(targetPath, relativePath);
    if (!fs5.existsSync(fullPath)) {
      continue;
    }
    const parsed = safeReadJsonFile(fullPath);
    if (!parsed) {
      continue;
    }
...
function collectProjectSignals(targetPath, allFiles) {
  ...
  const additionalTestFiles = [
    ...collectFilesRecursively(path6.join(targetPath, "test")),
    ...collectFilesRecursively(path6.join(targetPath, "tests")),
    ...collectFilesRecursively(path6.join(targetPath, "__tests__")),
    ...collectFilesRecursively(path6.join(targetPath, "cypress")),
    ...collectFilesRecursively(path6.join(targetPath, "playwright"))
  ];
  ...
  const namingStyles = sourceFiles.map((file) => detectFileNamingStyle(file.path)).filter((style) => style !== null);
  const namingCounts = namingStyles.reduce((acc, style) => {
  ...
  ].map((relativePath) => path6.join(targetPath, relativePath)).filter((fullPath) => fs5.existsSync(fullPath));
  const configTexts = configFiles.map((fullPath) => safeReadTextFile(fullPath)).filter((content) => Boolean(content));
  ...
  const lineBelow300Ratio = sourceFileCount === 0 ? 0 : sourceFiles.filter((file) => file.lines < 300).length / sourceFileCount;
  const readmeExists = ["README.md", "README.MD"].some((fileName) => fs5.existsSync(path6.join(targetPath, fileName)));
  const docsDirExists = fs5.existsSync(path6.join(targetPath, "docs"));
  const projectDocsExists = docsDirExists || fs5.existsSync(path6.join(targetPath, "PROJECT.md")) || fs5.existsSync(path6.join(targetPath, "ROADMAP.md"));
  ...
  return {
    ...
    eslintRulesCount,
    prettierConfigured: packageJson?.prettier !== void 0 || Boolean(findFirstExistingFile(targetPath, PRETTIER_CONFIG_FILES)),
    preCommitConfigured: fs5.existsSync(path6.join(targetPath, ".husky", "pre-commit")) || packageJson?.["lint-staged"] !== void 0 || packageJson?.["simple-git-hooks"] !== void 0,
    buildConfigured: Boolean(packageJson?.scripts?.build) || Boolean(findFirstExistingFile(targetPath, BUILD_CONFIG_FILES)),
    testFrameworkConfigured: Boolean(packageJson?.scripts?.test) || Boolean(findFirstExistingFile(targetPath, TEST_CONFIG_FILES)) || testFiles.size > 0,
    coverageConfigured: hasAnyScriptMatch(packageJson?.scripts, /coverage/i) || COVERAGE_REPORT_FILES.some((relativePath) => fs5.existsSync(path6.join(targetPath, relativePath))),
    coverageReportExists: COVERAGE_REPORT_FILES.some((relativePath) => fs5.existsSync(path6.join(targetPath, relativePath))),
    ...
    lockfileExists: [
      ...
      "bun.lock",
      "bun.lockb"
    ].some((fileName) => fs5.existsSync(path6.join(targetPath, fileName))),
...
function getDimensionStatus(score, weight) {
...
function createDimension(id, label, weight, criteria, measuredSummary, unmeasuredSummary) {
  ...
  const normalizedScore = rawMaxScore === 0 ? 0 : Math.max(0, Math.min(weight, Math.round(rawScore / rawMaxScore * weight)));
  return {
    id,
    label,
    weight,
...
function buildEngineeringScorecard(counts, breakdown, signals, hasFeatureDir) {
  const architecture = createDimension(
    ...
    counts.SA001 > 0 ? "\u5B58\u5728\u7C7B\u578B\u5206\u7EC4/\u7ED3\u6784\u6DF7\u7528\u4FE1\u53F7\uFF0C\u76EE\u5F55\u4ECD\u9700\u6536\u655B\u3002" : "\u7ED3\u6784\u89C4\u5219\u6574\u4F53\u7A33\u5B9A\uFF0C\u76EE\u5F55\u6DF1\u5EA6\u4FDD\u6301\u5728\u53EF\u63A7\u8303\u56F4\u5185\u3002",
    "\u672A\u68C0\u6D4B\u5230\u53EF\u7528\u4E8E\u5224\u65AD\u67B6\u6784\u7ED3\u6784\u7684\u6E90\u7801\u3002"
  );
  const lintCriterion = signals.lintViolationCount === null ? createCriterion("Lint \u8FDD\u89C4\u6570\u91CF", 7, 0, false, null, "\u672A\u53D1\u73B0 lint \u7ED3\u679C\u62A5\u544A\uFF0C\u672A\u7EB3\u5165\u6263\u5206") : createCriterion(
    "Lint \u8FDD\u89C4\u6570\u91CF",
    7,
  ...
  const codeQuality = createDimension(
    ...
    [
      createCriterion(
        ...
        signals.eslintConfigured ? `\u89C4\u5219\u6570 ${signals.eslintRulesCount ?? "\u672A\u89E3\u6790"}` : "\u672A\u68C0\u6D4B\u5230 ESLint \u914D\u7F6E"
      ),
  ...
  const testCoverage = createDimension(
    ...
    signals.testFilesCount > 0 ? "\u5DF2\u68C0\u6D4B\u5230\u6D4B\u8BD5\u6587\u4EF6\u4E0E\u5DE5\u5177\u94FE\u4FE1\u53F7\uFF0C\u53EF\u7EE7\u7EED\u63A5\u5165\u771F\u5B9E\u8986\u76D6\u7387\u6570\u636E\u3002" : "\u6D4B\u8BD5\u8D44\u4EA7\u8F83\u5F31\uFF0C\u5F53\u524D\u4E3B\u8981\u4F9D\u8D56\u914D\u7F6E\u7EA7\u4FE1\u53F7\u3002",
  ...
  const dependencyHealth = createDimension(
    ...
    [
      ...
      createCriterion(
        "\u7248\u672C\u58F0\u660E\u5065\u5EB7\u5EA6",
        5,
        signals.dependencyCount === 0 ? 0 : signals.wildcardDependencyCount === 0 ? 5 : signals.wildcardDependencyCount / signals.dependencyCount <= 0.1 ? 3 : 0,
        signals.dependencyCount > 0,
        signals.dependencyCount > 0 && signals.wildcardDependencyCount === 0,
        signals.dependencyCount > 0 ? `\u5BBD\u677E\u7248\u672C\u58F0\u660E ${signals.wildcardDependencyCount}/${signals.dependencyCount}` : "\u672A\u68C0\u6D4B\u5230\u4F9D\u8D56\u58F0\u660E"
      )
    ],
  ...
  const documentation = createDimension(
    ...
    [
      ...
      createCriterion(
        ...
        signals.sourceFileCount > 0 ? `\u542B\u6CE8\u91CA\u5757\u6587\u4EF6 ${signals.docCommentFileCount}/${signals.sourceFileCount}` : "\u65E0\u6E90\u7801\u6587\u4EF6"
      )
    ],
...
function calculateScores(violations, context, targetPath) {
  const counts = {
    SA001: 0,
    SA002: 0,
    SA003: 0,
...
function formatDimensionStatus(status) {
  switch (status) {
    case "excellent":
      return "\u2705 \u4F18\u79C0";
    case "good":
...
function formatMarkdown(result) {
  ...
  if (result.scores.total >= 90) {
    ...
    lines.push("\u{1F534} \u5DE5\u7A0B\u5065\u5EB7\u5EA6\u9700\u8981\u8F83\u5927\u6539\u8FDB\uFF1A");
...
function analyze(options) {
  ...
  const fullPath = path6.resolve(targetPath);
  if (!fs5.existsSync(fullPath)) {
    throw new Error(`\u76EE\u6807\u8DEF\u5F84\u4E0D\u5B58\u5728: ${fullPath}`);
  }
  const srcPath = path6.join(fullPath, srcDir);
  const scanPath = fs5.existsSync(srcPath) ? srcPath : fullPath;
...
function saveReports(targetPath, result) {
  try {
    ...
    const archDir = path6.join(reportsPath, "architecture");
    if (!fs5.existsSync(archDir)) {
      fs5.mkdirSync(archDir, { recursive: true });
    }
    const markdownContent = formatMarkdown(result);
    fs5.writeFileSync(path6.join(archDir, "latest.md"), markdownContent, "utf-8");
...
function main2() {
  ...
  try {
    ...
    if (!options.noSave) {
      saveReports(path6.resolve(options.targetPath), result);
    }
  } catch (error) {
