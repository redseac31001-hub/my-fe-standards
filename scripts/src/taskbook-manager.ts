...
export class TaskBookManager {
  ...
  load(id: string): TaskBook | null {
    ...
    if (fs.existsSync(filePath)) {
      ...
      return JSON.parse(content) as TaskBook;
    }
   ...
   * 更新 TaskBook 状态
   */
  updateStatus(id: string, status: TaskBookStatus): TaskBook | null {
   ...
   * 添加任务
   */
  addTask(id: string, task: Omit<TaskItem, 'id'>): TaskBook | null {
    const taskBook = this.load(id);
    if (!taskBook) return null;
  ...
  getParallelizableTasks(taskBookId: string): TaskItem[] {
    ...
    const completedTaskIds = new Set(
      taskBook.tasks.filter(t => t.status === 'done').map(t => t.id)
    );
