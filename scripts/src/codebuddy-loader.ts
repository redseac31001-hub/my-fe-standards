  ...
  if (ctx.isRemote) {
    ...
    for (const file of skillFiles) {
      ...
      try {
        ...
        logWarn(`技能文件下载失败: ${file.path} - ${(e as Error).message}`);
      }
    }
  } else {
    // 本地模式：复制技能文件
  ...
  if (ctx.isRemote) {
    ...
    for (const file of agentFiles) {
      ...
      try {
        ...
        logWarn(`Agent文件下载失败: ${file.path} - ${(e as Error).message}`);
      }
    }
  } else {
    // 本地模式：复制 Agent 文件
 ...
 * 分发可执行脚本到业务项目
 */
async function distributeScripts(targetDir: string): Promise<string[]> {
  ...
  if (ctx.isRemote) {
    ...
    for (const cmdFile of COMMANDS_TO_DISTRIBUTE) {
      ...
      try {
        ...
        logWarn(`命令下载失败: ${cmdFile} - ${(e as Error).message}`);
      }
    }
  } else {
    // 本地模式：从本地复制命令文件
 ...
 * 生成脚本目录的 README
 */
function generateScriptsReadme(scripts: string[]): string {
  const lines: string[] = [
    '# CodeBuddy 工具脚本',
    '',
    '> 自动生成，请勿手动编辑',
 ...
 * 生成脚本使用提示词
 */
function generateScriptsPrompt(scripts: string[]): string {
...
`;
  ...
  const layer1Rules = await loadLayerRules(layers.base?.id || 'layer1_base', layer1Folders);
  ...
  for (const [depName, ruleFolders] of Object.entries(businessDeps)) {
    if (dependencies[depName]) {
      ...
      for (const folder of ruleFolders) {
        ...
        const content = await loadRuleFile(layers.business?.id || 'layer2_business', folder + '.md');
        if (content) {
  ...
  for (const item of actionDefaults) {
    ...
    const content = await loadRuleFile(layers.action?.id || 'layer3_action', item + '.md');
    if (content) {
