...
function getCommandPromptEntry(cmd: string): CommandPromptEntry {
...
function getScriptPromptEntry(script: string): ScriptPromptEntry {
...
function buildCommandsTable(commands: string[]): string {
...
function buildScriptsTable(scripts: string[]): string {
...
function buildCommandsSummaryTable(commands: string[]): string {
  const entries = commands
    .map(getCommandPromptEntry)
    .sort((a, b) => a.command.localeCompare(b.command));
...
function buildScriptPromptGroups(scripts: string[]): ScriptPromptGroup[] {
  const scriptSet = new Set(scripts);
  const consumed = new Set<string>();
  const groups: ScriptPromptGroup[] = [];

  const addGroup = (title: string, summary: string, entry: string, files: string[]): void => {
...
function buildScriptsSummaryTable(scripts: string[]): string {
...
export function generateCommandsReadme(commands: string[]): string {
...
export function generateQuickActionGuide(): string {
  return `
## ⚡ 快速行动指引
...
对话式任务请求与 \`/task\` 是同一产品入口的两种表面形式：都必须先做 task-intake routing，再进入 TaskBook / Planner / Validator 闭环。系统会按复杂度自动分流到 \`direct\`、\`planner\`、\`task-orchestrator\`，并同步匹配 \`minimal\`、\`standard\`、\`full\` 三档文档留存级别。
...
function buildAgentHint(agent: AgentMetadata): string {
  const parts: string[] = [];
...
function groupAgentsByScenario(agents: AgentMetadata[]): Array<RouteGroup<AgentMetadata, AgentRouteCategory>> {
  const groups: Array<RouteGroup<AgentMetadata, AgentRouteCategory>> = [
    {
      key: 'orchestration',
      title: '计划与执行',
...
function classifySkillRouteCategory(skill: SkillMetadata): SkillRouteCategory {
...
function groupSkillsByScenario(skills: SkillMetadata[]): Array<RouteGroup<SkillMetadata, SkillRouteCategory>> {
  const groups: Array<RouteGroup<SkillMetadata, SkillRouteCategory>> = [
    {
      key: 'architecture',
      title: '架构与分析',
...
命中任务编排意图后，不要把对话式请求当作自由聊天；应与 \`/task\` 共用同一条 TaskBook / Planner / Validator 路由。系统会自动按复杂度分流到 \`direct\`、\`planner\`、\`task-orchestrator\`，并自动决定 \`minimal\`、\`standard\`、\`full\` 三档文档留存级别。
