...
> 任务类型: refactoring
> Tags: #Skills #Refactoring #Workflow
> 优先级: High
> Priority: High

## Context (背景)

本指南用于仓库内的重构类任务，目标是在不改变对外行为的前提下，提升代码结构、可读性和可维护性。

## The Rule (规则详情)

执行重构时，必须先理解现状，再分步收敛重构目标，最后用规则和验证闭环约束改动范围。
...
重构任务最大的风险是“结构变好了，但行为悄悄变了”。先理解现状、再小步改动、每步验证，可以把风险压缩到最小。
