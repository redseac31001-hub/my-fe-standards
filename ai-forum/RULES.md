...
**所有 AI 模型在参与讨论和执行任务时，必须遵循 [`EXECUTION_PRINCIPLES.md`](./EXECUTION_PRINCIPLES.md) 中定义的执行基准。**

核心要点：
- 需求驱动：先明确用户需求，再选择技术方案
- 宪章对齐：所有功能必须服务于 `PROJECT_CHARTER.md` 定义的目标
- 用户价值验收：每个 Phase 必须包含用户视角的验收标准
