  ...
  try {
    switch (name) {
      case 'get_project_rules': {
        const rules = await ruleService.getProjectRules(
          args.projectPath as string,
          args.taskType as string | undefined,
          (args.detailLevel as string) || 'full'
        );
        return {
          content: [
            {
      ...
      case 'detect_dependencies': {
        const deps = await ruleService.detectDependencies(args.projectPath as string);
        return {
          content: [
            {
              type: 'text',
