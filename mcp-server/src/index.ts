...
${next.notes ? `\n备注: ${next.notes}` : ''}
    ...
    case 'taskbook_report': {
      ...
      try {
        const cliArgs: string[] = ['report', parsed.data.taskBookId]
        if (parsed.data.write) cliArgs.push('--write')
        if (parsed.data.out) cliArgs.push('--out', parsed.data.out)
        cliArgs.push('--json')
        const output = execCodebuddyScript(dirResult.dir, scriptResult.scriptPath, cliArgs)
        return { content: [{ type: 'text', text: output }] }
      } catch (error) {
    ...
    case 'reports_inspect': {
      ...
      try {
        const cliArgs: string[] = ['inspect']
        if (parsed.data.module) cliArgs.push('--module', parsed.data.module)
        if (parsed.data.file) cliArgs.push('--file', parsed.data.file)
        if (typeof parsed.data.depth === 'number') cliArgs.push('--depth', String(parsed.data.depth))
        if (typeof parsed.data.trendPoints === 'number') cliArgs.push('--trend', String(parsed.data.trendPoints))
        cliArgs.push('--json')
        const output = execCodebuddyScript(dirResult.dir, scriptResult.scriptPath, cliArgs)
        return { content: [{ type: 'text', text: output }] }
      } catch (error) {
    ...
    case 'reports_hotspots': {
      ...
      try {
        ...
        const cliArgs: string[] = ['hotspots', '--top', String(top), '--json']
        const output = execCodebuddyScript(dirResult.dir, scriptResult.scriptPath, cliArgs)
        return { content: [{ type: 'text', text: output }] }
      } catch (error) {
